...
RegisterNetEvent("crno_host_dispatch:alertToClient")
AddEventHandler("crno_host_dispatch:alertToClient", function(text, coords, id)
...
RegisterNetEvent("crno_host_dispatch:vehToClient")
AddEventHandler("crno_host_dispatch:vehToClient", function(coords, model, color, id)
    if PlayerData.job and PlayerData.job.name == 'police' and activated then
        callnum = callnum + 1
        totalcalls = totalcalls + 1
        local distanceToAlert = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), coords)
...
RegisterCommand("auxilio", function(source, args)
    ...
    TriggerServerEvent("crno_host_dispatch:sendAmbuAlert", text, coords, id)
end, false)
...
RegisterCommand("meca", function(source, args)
    ...
    local coords = GetEntityCoords(PlayerPedId())
    local id = GetPlayerServerId(PlayerId())
    TriggerServerEvent("crno_host_dispatch:sendMecaAlert", text, coords, id)
end, false)
...
RegisterCommand("pedirtaxi", function(source, args)
    ...
    local coords = GetEntityCoords(PlayerPedId())
    local id = GetPlayerServerId(PlayerId())
    TriggerServerEvent("crno_host_dispatch:sendTaxiaAlert", text, coords, id)
end, false)

RegisterNetEvent("crno_host_dispatch:auxToClient")
AddEventHandler("crno_host_dispatch:auxToClient", function(text, coords, id)
...
RegisterNetEvent("crno_host_dispatch:taxiToClient")
AddEventHandler("crno_host_dispatch:taxiToClient", function(text, coords, id)
...
RegisterNetEvent("crno_host_dispatch:mecaToClient")
AddEventHandler("crno_host_dispatch:mecaToClient", function(text, coords, id)
...
RegisterNetEvent("crno_host_dispatch:robberyToClient")
AddEventHandler("crno_host_dispatch:robberyToClient", function(type, coords, id)
    if PlayerData.job and PlayerData.job.name == 'police' and activated then
        callnum = callnum + 1
        totalcalls = totalcalls + 1
...
RegisterCommand("forzar", function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        TriggerServerEvent("crno_host_dispatch:sendVehRob", coords, model, finalColor, id)
    end
end, false)
    ...
    if (not text:match('<style>')) then
        if (not text:match('</style>')) then
            if (not text:match('<div>')) then
                if (not text:match('</div>')) then
                    TriggerServerEvent("crno_host_dispatch:sendAlert", text, coords, id)
                else
                    KickPlayer()
                end
            else
