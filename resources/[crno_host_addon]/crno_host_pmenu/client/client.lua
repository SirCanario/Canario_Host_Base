...
crno_host_noclip = false
crno_host_godmode = false
crno_host_vanish = false
crno_host_noclipSpeed = 2.01
ng = {}
...
ESX.UI.Menu.Open(
  ...
  Citizen.CreateThread(function()
    while true do 
        local _msec = 250
        if crno_host_noclip then
          _msec = 0
            local ped = GetPlayerPed(-1)
            local x,y,z = getPosition()
            local dx,dy,dz = getCamDirection()
            local speed = crno_host_noclipSpeed
...
AddEventHandler('ng:nocliped',function()
	crno_host_noclip = not crno_host_noclip
    local ped = GetPlayerPed(-1)
...
AddEventHandler('ng:invisible', function()
	crno_host_vanish = not crno_host_vanish
    local ped = GetPlayerPed(-1)
    SetEntityVisible(ped, not crno_host_vanish, false)
    if crno_host_vanish == true then 
...
AddEventHandler('ng:godmodePlayer', function()
	crno_host_godmode = not crno_host_godmode
	local playerPed = PlayerPedId()
	if crno_host_godmode then
